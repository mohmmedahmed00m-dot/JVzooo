...
export class LicensingService {
  ...
  verifyJvzooSignature(payload: JvzooIpnPayload, secret: string): boolean {
    ...
    // JVZIPN spec: each sorted field value followed by "|", then the secret key,
    // hashed with SHA1, then the first 8 hex chars uppercased.
    const concatenated = keys.map((k) => (fields[k] ?? '') + '|').join('') + secret;
    const fullHash = createHash('sha1').update(concatenated, 'utf8').digest('hex');
    const computed = fullHash.slice(0, 8).toUpperCase();
