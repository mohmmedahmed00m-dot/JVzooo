...
const jvzooSign=(f,s)=>{const{cverify,...r}=f;const k=Object.keys(r).sort();const c=k.map(x=>(r[x]??'')+'|').join('')+s;return crypto.createHash('sha1').update(c,'utf8').digest('hex').slice(0,8).toUpperCase();};

// Mirrors EncryptionService.decrypt() (src/common/crypto/encryption.service.ts) EXACTLY —
// same algorithm, same salt, same layout — so this script can recover the real
// plaintext license key from the `licenses.license_key` column, instead of using
// the raw AES-256-GCM ciphertext ($aes256gcm$...) that column actually holds.
// (The plaintext key is otherwise only ever available via the customer email.)
function decryptLicenseKey(ciphertext){
  if(!ciphertext || !ciphertext.startsWith('$aes256gcm$')) return ciphertext; // unencrypted/back-compat
  const IV_LEN=12, TAG_LEN=16;
  const encryptionKey = process.env.ENCRYPTION_KEY || 'dev-insecure-encryption-key-rotate-me';
  const salt = 'JVzo-AES256-GCM-KeyDerivation-Salt-2024';
  const key = crypto.scryptSync(encryptionKey, salt, 32);
  const buf = Buffer.from(ciphertext.slice('$aes256gcm$'.length), 'base64');
...
(async()=>{
  ...
  await client.end();
  const lk=decryptLicenseKey(lkRes.rows[0].license_key);
  ...
  r=await req('POST','/auth/activate-license',{token:token2,body:{license_key:lk}});
  check('SETUP: license activated with real (decrypted) key -> 200/201', r.status===200||r.status===201, 'status='+r.status+' body='+JSON.stringify(r.json).slice(0,100));
